...
namespace minipkg2 {
    ...
    // to_file()
    template<class It>
    static std::string print_range(std::string_view name, const It begin, const It end) {
        if (begin == end) {
            return fmt::format("{}=()\n", name);
        }

        auto it = begin;
        std::string str = fmt::format("{}=('{}'", name, *it);
        for (++it; it != end; ++it) {
            str += fmt::format(" '{}'", *it);
        }
        return str + ")\n";
    ...
    std::string binary_package_info::to_file() const {
        ...
        str += print_range("rdepends",              begin(rdepends), end(rdepends));
        str += print_range("provides",              begin(provides), end(provides));
        str += print_range("conflicts",             begin(conflicts), end(conflicts));
